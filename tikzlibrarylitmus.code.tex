...
\def\ifhex#1{\ifstrempty{#1}{\@secondoftwo}{\@ifhex-#1-0x\lit@del}}
\def\@ifhex#1-0x#2\lit@del{\ifstrempty{#1}}
\def\@gobblehex 0x#1\lit@del{#1}

\def\ifzero#1{\ifstrempty{#1}{\@secondoftwo}{\@ifzero-#1-0\lit@del}}
\def\@ifzero#1-0#2\lit@del{\ifstrempty{#1}}
\def\@gobblezeros #1\lit@del{\ifzero{#1}{\expandafter\@gobblezeros\@gobblezero #1\lit@del\lit@del}{#1}}
\def\@gobblezero 0#1\lit@del{#1}

\litmusset{
  symbol footprint/.code args={#1 from #2 to #3}{%
    \tikzmath{
      \litsymstart{#1} = #2;
      \litsymend{#1} = #3;
    }%
    \csdef{lit@sym@fp:#1}{exists}%
  },
    ...
    /litmus/memfp/.code n args={4}{%
      \ifcsdef{lit@sym@fp:##1}{%
        \tikzmath{
          ...
          print{\litmusset{event equal}};
          let \cond = \ifhex{##4}{1}{0};
          if \cond then {
            print{0x};
            let \addrstart = \litsymstart{##1};
            let \addrend = \litsymend{##1};
            if \offs + \size <= \addrend then {
              for \i in {\offs + \size,...,\addrend}{
                print{\_\_};
              };
            };
            print{\@gobblehex##4\lit@del};
            if \addrstart + 1 <= \offs then {
              for \i in {\addrstart + 1,...,\offs}{
                print{\_\_};
              };
            };
          } else {
        ...
        \ifhex{##4}{%
          \edef\lit@val{\@gobblehex##4\lit@del}%
          \edef\lit@val{\expandafter\@gobblezeros\lit@val\lit@del}%
          \edef\lit@val{\expandafter\ifstrempty\expandafter{\lit@val}{0}{\lit@val}}%
          \pgfmathbasetodec{\lit@val}{\lit@val}{16}%
          \lit@val%
        }{% else
          ##4%
        }%
...
\litmusset{
  ...
  compute assem text width/.style={
    ...
    /utils/ifboolexpe={test {\ifcsdef{#1@litmusinstructionswidths@uid}}} then {
      /utils/package warning={litmus-tikz-library}{'compute assem text width' expects a unique identifier, '#1' is already used.},
    } else {
...
\litmusset{
  ...
  init below/.style={
    /litmus/event init-#1/.append style={/tikz/yshift=-1.25ex, /tikz/below=of #1},
  },
  ...
  init below right/.style={
    /litmus/event init-#1/.append style={/tikz/below right=of #1},
    /litmus/(init-#1) rf (#1)/.append style=/tikz/swap,
  },
...
\litmusset{
  ...
  mem event/.style args={#1:#2 #3+#4/#5=#6}{
    ...
    /litmus/event contents/.get=\lit@event@contents,
    /tikz/node contents/.expand once=\lit@event@contents,
  },
  mem access/.code={#1},
}
...
\litmusset{%
  ...
  % Back edges:
  edge distance {-3,-1}/.style={/tikz/out=-160, /tikz/in=-60, /tikz/looseness=0.8, /litmus/label pos=0.9},
...
\litmusset{
  ...
  table header/long name/.style={
    node contents={\rlap\lit@name\hfill\llap{\raisebox{2ex}{\lit@arch}}},
  },
  ...
  table construction/.style={
    /litmus/table header={/tikz/above=1pt of threads box, /tikz/anchor=south},
