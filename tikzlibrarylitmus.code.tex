...
\litmusset{
  old-mixed-size/.style={
    ...
    /litmus/memrf/.code 2 args={%
      \def\memrf@temp{\@gobble}%
      \foreach \i/\j in {##2} {%
        \tikzmath{\size = int(\j - \i + 1);}%
        \ifnum\i=\z@%
          \xdef\memrf@temp{\memrf@temp,##1/\size}%
        \else%
          \xdef\memrf@temp{\memrf@temp,##1+\i/\size}%
        \fi%
      }%
      rf\,\memrf@temp%
    },
...
\litmusset{
  mixed-size/.style={
    ...
    /litmus/memrf/.code 2 args={%
      ...
      \foreach \i/\j in {##2} {%
        \ifx\i\j%
          \xdef\memrf@temp{\memrf@temp,\i}%
        \else%
          \xdef\memrf@temp{\memrf@temp,\i..\j}%
        \fi%
      }%
      rf\,##1[\memrf@temp]%
    },
  ...
  add events edge/.style args={(#1) #2 (#3)=#4}{
    ...
    /litmus/add to event relations/.append={%
      \ifboolexpr{%
        ...
        \path node[gobble init/.style args={init-##1}{/litmus/init=##1},gobble init=#1] {} edge[/litmus/.cd scope={#2,#4}] (#3);%
      }{% else
          \path (#1) edge[/litmus/.cd scope={#2,#4}] (#3);%
      }%
    },
  },
}
...
\newcommand{\inputlitmus}[3][]{%
  ...
  \else%
    ...
    \begin{scope}
      \node[/litmus/missing file] {??missing .tikz file??};%
    \end{scope}%
  \fi%
}
